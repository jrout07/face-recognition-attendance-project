...
/* ------------------ Root ------------------ */
app.get("/", (req, res) => res.send("✅ Backend Running with Finalized Attendance Support"));
...
/* ------------------ Teacher Refresh/Get Session ------------------ */
app.get("/teacher/getSession/:classId", async (req, res) => {
  const { classId } = req.params;
  try {
    const data = await dynamoDB.send(new ScanCommand({ TableName: process.env.DYNAMODB_SESSIONS_TABLE }));
    const session = data.Items?.find((s) => s.classId === classId && !s.finalized);
    if (!session) return res.json({ success: false, error: "No active session" });

    // Refresh QR token
    const qrToken = uuidv4();
    session.qrToken = qrToken;
    session.qrExpiresAt = new Date(Date.now() + 20 * 1000).toISOString();
    ...
    await dynamoDB.send(
      new UpdateCommand({
        ...
        ExpressionAttributeValues: { ":q": qrToken, ":e": session.qrExpiresAt },
      })
    );
